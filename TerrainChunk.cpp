...
void TerrainChunk::Initialize(const std::vector<float>& heightmap, int heightmapWidth, std::unordered_map<std::string, Texture*>& textures) {
    ...
    m_heightmapTexture = std::make_unique<Texture>(imageData, XMFLOAT2(1024, 1024));
    ...
    for (int z = 0; z < scalePatchZ; ++z) {
        for (int x = 0; x < scalePatchX; ++x) {
            ...
            int sampleZ = z * tessFactor;

            int noiseIndex = sampleZ * m_size + sampleX;
...
//void TerrainChunk::Initialize(const std::vector<float>& heightmap, int heightmapWidth, std::unordered_map<std::string, Texture*>& textures) {
//    int tessFactor = 4;
//    int vertsPerSide = m_size / tessFactor;
//    int totalVerts = vertsPerSide * vertsPerSide;
//
//    std::vector<VertexPosition> m_vertices(totalVerts);
...
//
//    // World-space offset
//    XMFLOAT3 worldOffset = GetWorldPosition();
//
//    for (int z = 0; z < vertsPerSide; ++z) {
//        for (int x = 0; x < vertsPerSide; ++x) {
//            int heightmapX = x * tessFactor;
//            int heightmapZ = z * tessFactor;
//            int heightmapIndex = heightmapZ * m_size + heightmapX;
//
//            // Guard: Prevent access out of bounds
//            if (heightmapIndex >= heightmapLocal.size()) continue;
//
//            // Sample local Perlin noise value and normalize it
//            float height = (heightmapLocal[heightmapIndex] + 1.0f) * 0.5f * m_heightScale;
//
//            int vertIndex = z * vertsPerSide + x;
//            m_vertices[vertIndex].Position = XMFLOAT3(
//                worldOffset.x + static_cast<float>(x * tessFactor),
//                height,
//                worldOffset.z + static_cast<float>(z * tessFactor)
//            );
//        }
//    }
//
//    // Generate quad patches (4 control points per patch)
...
//{
//    int tessFactor = 4;
//    int vertsPerSide = m_size / tessFactor; // e.g. 1024 / 4 = 256
//    int heightmapSize = vertsPerSide;
//
//    // One vertex per patch corner, one height per vertex
//    std::vector<float> heightmapLocal = GenerateChunkHeightmap(
//        FastNoiseLite::NoiseType_OpenSimplex2,
//        m_chunkX, m_chunkZ,
//        1024, 1024,
//        0.2f // noise scale, tweak this
//    );
//
//    std::vector<uint8_t> imageData(heightmapLocal.size() * 4); // RGBA for each pixel
//
...
//
//    m_heightmapTexture = std::make_unique<Texture>(imageData, XMFLOAT2(1024, 1024));
//    //Texture* Heightmap = new Texture(imageData, XMFLOAT2(heightmapSize, heightmapSize));
//
//    std::vector<VertexPosition> m_vertices(heightmapSize * heightmapSize);
//    std::vector<UINT> m_indices;
//
//    // World offset for the chunk
//    XMFLOAT3 worldOffset = GetWorldPosition();
//
...
//        {
//            int index = z * heightmapSize + x;
//
//            float height = (heightmapLocal[index] + 1.0f) * 0.5f * m_heightScale;
//
//            m_vertices[index].Position = XMFLOAT3(
//                worldOffset.x + x * tessFactor,
//                height,
//                worldOffset.z + z * tessFactor
//            );
//        }
//    }
//
//    // Quad patch indices (4 verts per patch)
//    for (int z = 0; z < heightmapSize - 1; ++z)
//    {
//        for (int x = 0; x < heightmapSize - 1; ++x)
//        {
//            UINT v0 = x + z * heightmapSize;
//            UINT v1 = (x + 1) + z * heightmapSize;
//            UINT v2 = x + (z + 1) * heightmapSize;
//            UINT v3 = (x + 1) + (z + 1) * heightmapSize;
//
//            m_indices.push_back(v0);
//            m_indices.push_back(v1);
//            m_indices.push_back(v2);
    ...
    float noiseScale)
{
    ...
    for (int z = 0; z < height; ++z)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            float worldX = static_cast<float>(chunkX * width + x * 4);
            float worldZ = static_cast<float>(chunkZ * height + z * 4);

            // Scale it into noise space
            float nx = worldX * noiseScale;
            float nz = worldZ * noiseScale;

            heightmap[z * width + x] = m_Noise.GetNoise(nx, nz);
        }
    }
